# Sync Timer (Room-based) — Project Plan / Requirements (MVP)
...
* Server that can:
    ...
    * broadcast `START` commands with an **absolute** server timestamp (`start_at`)
...
* Instead, server computes a common absolute time:
    ...
    * `start_at = server_now_ms + max(requested_delay_ms, min_lead_ms)`
* Server broadcasts `{ start_at }` to all room members.
* Each client renders countdown using:

    * `true_now = client_now + offset_ms`
    * `ms_left = start_at - true_now`
...
* `pnpm dev` (or `npm run dev`) runs both client and server
* `apps/server`:
